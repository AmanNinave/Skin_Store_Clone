


var arr = [
    {
        name: "SkinMedica AHA/BHA Exfoliating Cleanser (6oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289657-9384876690289403.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289657-7504876690381737.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "48",
        sellingPrice: "38.4",
        Category: "Clenser",
        rating: "4.8",
        link: "https://www.skinstore.com/skinmedica-aha-bha-exfoliating-cleanser/11289657.html"
    },
    {
        name: "SkinMedica Procedure 360 System 38oz (Worth $410)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/12062429-2064870995672366.jpg",
        alt_url: "",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "326",
        sellingPrice: "260.8",
        Category: "Clenser",
        rating: "4.1",
        link: "https://www.skinstore.com/skinmedica-procedure-360-system-38oz/12062429.html"
    },
    {
        name: "SkinMedica Purifying Foaming Wash 5oz",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11325073-1564871019659259.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11325073-4104864824880119.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "46",
        sellingPrice: "36.8",
        Category: "Clenser",
        rating: "3.3",
        link: "https://www.skinstore.com/skinmedica-purifying-foaming-wash-5oz/11325073.html"
    },
    {
        name: "SkinMedica Facial Cleanser",
        img_url: "https://static.thcdn.com/images/large/webp/productimg/1600/1600/11289661-1794453993724103.jpg",
        alt_url: "",
        gift: "20% off + $135 SkinMedica Gif",
        StrikedoffPrice: "40",
        sellingPrice: "32",
        Category: "Clenser",
        rating: "4.5",
        link: "https://www.skinstore.com/skinmedica-facial-cleanser/11289661.html"
    },
    {
        name: "SkinMedica AHA/BHA Exfoliating Cleanser (6oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289657-9384876690289403.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289657-7504876690381737.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "48",
        sellingPrice: "38.4",
        Category: "Exfoliator",
        rating: "4.8",
        link: "https://www.skinstore.com/skinmedica-aha-bha-exfoliating-cleanser/11289657.html"
    },
    {
        name: "SkinMedica AHA/BHA Cream (2oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289656-1194870995967011.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289656-7314866614829336.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "46",
        sellingPrice: "36.8",
        Category: "Exfoliator",
        rating: "",
        link: "https://www.skinstore.com/skinmedica-aha-bha-cream/11289656.html"
    },
    {
        name: "SkinMedica TNS Eye Repair (0.5oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289683-2074871019620370.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289683-1424866649587504.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "106",
        sellingPrice: "84.4",
        Category: "Eyecare",
        rating: "3.5",
        link: "https://www.skinstore.com/skinmedica-tns-eye-repair/11289683.html"
    },
    {
        name: "SkinMedica Instant Bright Eye Cream 5 oz",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/12378444-2084870995827076.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/12378444-4184870995881904.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "92",
        sellingPrice: "73.6",
        Category: "Eyecare",
        rating: "",
        link: "https://www.skinstore.com/skinmedica-instant-bright-eye-cream-5-oz/12378444.html"
    },
    {
        name: "SkinMedica Instant Bright Eye Mask (6 Pack)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/12455153-5734870995929202.jpg",
        alt_url: "",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "50",
        sellingPrice: "40",
        Category: "Eyecare",
        rating: "3.6",
        link: "https://www.skinstore.com/skinmedica-instant-bright-eye-mask-6-pack/12455153.html"
    },
    {
        name: "SkinMedica Scar Recovery Gel (0.5oz)",
        img_url: "https://static.thcdn.com/images/large/webp/productimg/1600/1600/11325044-1034453995033423.jpg",
        alt_url: "",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "46",
        sellingPrice: "36.8",
        Category: "Gel",
        rating: "2.8",
        link: "https://www.skinstore.com/skinmedica-scar-recovery-gel-0.5oz/11325044.html"
    },
    {
        name: "SkinMedica Scar Recovery Gel with Centelline (2oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289679-1194871019622113.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289679-2344871019662661.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "106",
        sellingPrice: "84.8",
        Category: "Gel",
        rating: "3.9",
        link: "https://www.skinstore.com/skinmedica-scar-recovery-gel-2oz/11289679.html"
    },
    {
        name: "SkinMedica HA5 Smooth & Plump Lip System",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11422018-5884870995772050.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11422018-1224870995824166.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "68",
        sellingPrice: "54.4",
        Category: "Lipcare",
        rating: "4.4",
        link: "https://www.skinstore.com/skinmedica-ha5-smooth-plump-lip-system/11422018.html"
    },
    {
        name: "SkinMedica HA5 Rejuvenating Hydrator and Plump Lip System (Worth $246)",
        img_url: "https://static.thcdn.com/images/large/webp/productimg/1600/1600/11492786-1224494372715692.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp/productimg/1600/1600/11492786-6904494372748710.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "241",
        sellingPrice: "192.8",
        Category: "Lipcare",
        rating: "2.4",
        link: "https://www.skinstore.com/skinmedica-ha5-rejuvenating-hydrator-and-ha5-smooth-and-plump-lip-system/11492786.html"
    },
    {
        name: "SkinMedica TNS Advanced+ Serum 1oz",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/12596429-3594871019700125.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/12596429-7834775946032493.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "295",
        sellingPrice: "236",
        Category: "Serum",
        rating: "4.7",
        link: "https://www.skinstore.com/skinmedica-tns-advanced-serum-1oz/12596429.html"
    },
    {
        name: "SkinMedica HA5 Rejuvenating Hydrator",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11290631-5864870995720591.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11290631-2134870995771630.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "184",
        sellingPrice: "147.2",
        Category: "Serum",
        rating: "4.9",
        link: "https://www.skinstore.com/skinmedica-ha5-rejuvenating-hydrator/11290631.html"
    },
    {
        name: "SkinMedica LYTERA 2.0 Pigment Correcting Serum",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11396212-1194870995675144.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11396212-3044875697859550.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "160",
        sellingPrice: "128",
        Category: "Serum",
        rating: "4.3",
        link: "https://www.skinstore.com/skinmedica-lytera-2.0-pigment-correcting-serum/11396212.html"
    },
    {
        name: "SkinMedica LYTERA 2.0 Pigment Correcting Serum and Retinol Complex 0.5 (Worth $231)",
        img_url: "https://static.thcdn.com/images/large/webp/productimg/1600/1600/11492784-6204494372463164.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp/productimg/1600/1600/11492784-1364494372613290.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "226.5",
        sellingPrice: "181.2",
        Category: "Serum",
        rating: "2.8",
        link: "https://www.skinstore.com/skinmedica-lytera-2.0-pigment-correcting-serum-and-retinol-complex-0.5/11492784.html"
    },
    {
        name: "SkinMedica Essential Defense Mineral Shield Broad Spectrum SPF 32 Tinted (1.85oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11290741-2554870995987858.jpg",
        alt_url: "https://static.thcdn.com/images/small/webp//productimg/130/130/11290741-3044870996038549.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "40",
        sellingPrice: "32",
        Category: "Moisturizer",
        rating: "4.1",
        link: "https://www.skinstore.com/skinmedica-essential-defense-mineral-shield-broad-spectrum-spf-32-tinted/11290741.html"
    },
    {
        name: "SkinMedica TNS Ceramide Treatment Cream (2oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289681-1484871019698619.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289681-1904871019505803.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "72",
        sellingPrice: "57.6",
        Category: "Moisturizer",
        rating: "2.3",
        link: "https://www.skinstore.com/skinmedica-tns-ceramide-treatment-cream/11289681.html"
    },
    {
        name: "SkinMedica Dermal Repair Cream",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289659-3844870996065429.jpg",
        alt_url: "https://static.thcdn.com/images/small/webp//productimg/130/130/11289659-2694864824207573.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "134",
        sellingPrice: "107.2",
        Category: "Moisturizer",
        rating: "2.9",
        link: "https://www.skinstore.com/skinmedica-dermal-repair-cream/11289659.html"
    },
    {
        name: "SkinMedica Total Defense and Repair SPF 34 (2.3oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289687-1484871029689150.jpg",
        alt_url: "https://static.thcdn.com/images/large/webp/productimg/1600/1600/11289687-1924410181573815.jpg",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "70",
        sellingPrice: "56",
        Category: "Moisturizer",
        rating: "4.3",
        link: "https://www.skinstore.com/skinmedica-total-defense-and-repair-spf-34/11289687.html"
    },
    {
        name: "SkinMedica AHA/BHA Cream (2oz)",
        img_url: "https://static.thcdn.com/images/large/webp//productimg/1600/1600/11289656-1194870995967011.jpg",
        alt_url: "",
        gift: "20% off + $135 SkinMedica Gift",
        StrikedoffPrice: "46",
        sellingPrice: "36.8",
        Category: "Moisturizer",
        rating: "5",
        link: "https://www.skinstore.com/skinmedica-aha-bha-cream/11289656.html"
    }

];


displayData(arr);

function displayData(showdata){

showdata.map(function (elem) {

    var skbox = document.createElement("div");

    var imgage = document.createElement("img");
    imgage.setAttribute("src", elem.img_url);

    var namebox = document.createElement("div");

    var nme = document.createElement("h4");
    nme.textContent = elem.name;


    var disc = document.createElement("p");
    disc.textContent = elem.gift;

    var priceBox = document.createElement("div");

    var sop = document.createElement("P");
    sop.textContent = " MSRP: $ " + elem.StrikedoffPrice;

    var sp = document.createElement("p");
    sp.textContent = "$ " + elem.sellingPrice;


    var btn = document.createElement("button")
    btn.textContent = "Add to Cart";
    btn.addEventListener("click", function () {
        addCart(elem);
    });


    namebox.append(nme);
    priceBox.append(sop, sp);
    skbox.append(imgage, namebox, disc, priceBox, btn);
    document.getElementById("ProductsDisplayGrid").append(skbox);

});

}




// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Adding Element to cart >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

var AddToCart = JSON.parse(localStorage.getItem("cartdata")) || [];

// var MCart = JSON.parse(localStorage.getItem("MWCart")) || [];


var rs =-1;
function addCart(elem) {
    alert("Item Added to Cart");
    location.reload();
    var check = false;
    var idxc = -1;
    AddToCart.map(function (ele, idx) {

        if (ele.name == elem.name) {
            check = true;
            idxc = idx;
        }

    });

    rs =  Number(elem.qty);
    

    if (check) {
        elem.qty = Number(elem.qty) + Number(1);
        AddToCart.splice(idxc, 1);
    } else {
        elem.qty = 1;
    }

    AddToCart.push(elem);

    localStorage.setItem("cartdata", JSON.stringify(AddToCart));

    // location.reload();

    console.log(rs);
}
console.log(rs);



// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Adding Element to cart >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

document.querySelector("#p30-50").addEventListener("change", priceFilter);
function priceFilter() {

    var temp = document.querySelector("#p30-50").value;

    var PriceData = arr.filter(function (elem) {

      return elem.sellingPrice >= 20 && elem.sellingPrice <= 50;

    })
    
    displayData(PriceData);

    // if (temp == "") {
    //   displayList(JobData);
    // } else {
    //   displayList(roleData);
    // }

  }